...
boolean NewMessage = false; // whether or not the client was connected previously
//-------------------
...
const int CycleVersion = 4;

// Varian GC
const bool IsVarianGC = false;  // FW 2.4. True for eTrap_FW_2.4_BrukerVarianGC.hex  flase for eTrap_FW_2.4.hex

//	For Hardware Simulation:  Set both to true!
bool FAKE_SYNC_SIGNALS					    = false;			  // Not sent from PC !
bool FAKE_TEMPERATURE_AND_HEATERS		= false;
// Set time that simulates waiting for a Sync Signal
unsigned long FAKE_SYNC_SINGALS_WAIT_SECONDS = 10000 ;	// 10 seconds waiting for each Sync Signals;			
...
float	HW_Maximum_Temperature_with_Cool	=	15;

unsigned long	HW_Cryotimeout_Millis		=	3 * 60000;	//3 min  * 60 * 1000
...
float TempCorrectionSlope			= 1;	
float TempCorrectionSlope_LowTemp	= 1;	// currently used
float TempCorrectionSlope_MidTemp	= 1;
float TempCorrectionSlope_HiTemp	= 1;	// currently used
...
//	Cycle and Controler Status
	...
	bool STATUS_WAIT_FOR_PAL_INCUBATION		= false;
...
//	PAL Incubation Time needed for waiting FIRST RUN ONLY
	unsigned long	PAL_incubationTime		= 30;
	bool			isFirstSequenceRun		= true;
	const int		waitPALInc_timer		= 3;
...
void setup() 
{
  ...
  Serial.println(Ethernet.localIP());
	...
	if (IsVarianGC)		set_pin_GC_Ready(OFF);	// Varian GC: Ready for PAL
	else				      set_pin_GC_Ready(ON);
...
//-----------------
  ...
  if (client)
  {
    if (!alreadyConnected)
    {
      Serial.println("We have a new client and answer 'Hello Client'");      
      client.println("Hello, client!");   // Ack to Client          
...
//------------------
	...
	{   
		if ( FAKE_TEMPERATURE_AND_HEATERS )													// FAKE_TEMPERATURE_AND_HEATERS
		...
		else
		{
			if (STATUS_SYSTEM_IS_SECURE)  
				...
				{	
					...
					STATUS_CYCLE_RESTART_PLEASE		= true;		// force reset if all is ok
				}
...
void Do_CreateCycle()
	{
		Line = 0;	ATOM[Line] = SET_VARIABLE;		PARM1[Line] = PAL_INCUBATION_TIME;	PARM2[Line] = 45;		PARM3[Line] = 0;		// 0 
		...
		Line++;		ATOM[Line] = SET_TEMP;			PARM1[Line] = 50;					PARM2[Line] = 10;		PARM3[Line] = 0;		// 6 Security Temp = Standby Temp
...
void Do_WorkOnCycle()
	{	
		...
		int Tf		= fakeSyncSignals_timer;
		int TInc	= waitPALInc_timer;
		...
		if (STATUS_CYCLE_RESTART_PLEASE)
		{
			...
			STATUS_CYCLE_RESTART_PLEASE		= false;
			STATUS_WAIT_FOR_TEMP			= false;
			STATUS_WAIT_FOR_PAL_INCUBATION	= false;
		}
		...
		switch (ATOM[Line])
		{
			...
			case SET_VARIABLE:
					if ( PARM1[Line] == PAL_INCUBATION_TIME )				//
						PAL_incubationTime = (unsigned long)PARM2[Line];						
			...
			case WAIT_SYNC_SIG:			// 2 Parms	Wait_Sync_Signal	(Signal, Signal State )
						switch (PARM1[Line]) 
						{
								case PREPARE_2:
									...
									{	if ( !TIMER_IS_STARTED[Tf] ) 				
										{	WAIT_TIME_OF_TIMER[Tf] = (FAKE_SYNC_SINGALS_WAIT_SECONDS * 1000);
											START_TIME_OF_TIMER[Tf] = Milliseconds;
											TIMER_IS_STARTED[Tf] = true;

											
										}
										else
										{	if (CURRENT_TIMER_TIME[Tf] >= WAIT_TIME_OF_TIMER[Tf] )
											{	TIMER_IS_STARTED[Tf]		= false;
												CURRENT_TIMER_TIME[Tf]		= 0;
												WAIT_TIME_OF_TIMER[Tf]		= 0;
												_prepare2Found			= true;
											}
										}
									}
						...
						case GC_START:
								...
								{	
									if ( TIMER_IS_STARTED[Tf] == false )						// FAKE_SNYC_SIGNALS
									{	
										WAIT_TIME_OF_TIMER[Tf] = (FAKE_SYNC_SINGALS_WAIT_SECONDS * 1000);
										START_TIME_OF_TIMER[Tf] = Milliseconds;
										TIMER_IS_STARTED[Tf] = true;
                        ...
                        if (IsVarianGC) 
                            set_pin_GC_Ready(OFF);
								...
								else
								{
									...
									{	
											if ((state_GC_Prepare == ON) || (IGNORE_GC_PREPARE == true))			
											{	
												// IsVarianGC?
												if (IsVarianGC) 
												    set_pin_GC_Ready(OFF);
...
void OnAskForReadbacks() {
	...
	cmdMessenger.sendCmdArg(heatUpSlope);
	...
	//cmdMessenger.sendCmdArg(CURRENT_TIMER_TIME[3]);
	cmdMessenger.sendCmdArg(WAIT_TIME_OF_TIMER[4]);
	cmdMessenger.sendCmdArg(CURRENT_TIMER_TIME[4]);

	cmdMessenger.sendCmdArg((int)MESSAGE_CRYO_TIMEOUT_TO_PC);

	cmdMessenger.sendCmdArg((int)PAL_incubationTime);
	...
	{	
		...
		{	
			...
			case (int)pChangeVariables:
				...
				// version 2	add PAL_incubationTime
				PARM2[0]	=	item[7];		//PAL_incubationTime
				PAL_incubationTime = item[7];				
...
void _restartCycle()
{	
	if (IsVarianGC)		set_pin_GC_Ready(OFF);			// Varian GC: Set TTL Low for PAL
	else				      set_pin_GC_Ready(ON);				// allow GC to get ready
	STATUS_CYCLE_RESTART_PLEASE = true;
	STATUS_CYCLE_RUNNING		= true;
	MESSAGE_CRYO_TIMEOUT_TO_PC	= false;

	isFirstSequenceRun		= true;		// need to use PAL_Incubation Time
}
...
{	
	...
	// 1. This is for Heatup during Cycle to High Temp
	...
	// 2. This is for ALL Temps (except above)
	if ( goalTemperature < 150 )	// = Standby Temp and Low Temperature and mannually Set
	{	
		_proportional			= 300;
		_integral				= 0;
		_integral_on			= false;
	...
	// Correct with built-in corrections	
	TempCorrectionSlope	= TempCorrectionSlope_MidTemp;
}

// Do the P I D stuff
void update_pid()
{	// all _vars are global	
	if ( state_TRAP_Cool < 1 )					// Heating
...
//float currentHeatRate			= 0;	// calced from lastHeatRateTemp / currentHeatRateTemp
//float rateToSwitchOnIntegral	= 0;	// If rate is lower :  Swicth on the Integral
void updateHeaingRateIssues()
{
	if ( (goalTemperature > 150) && (!_integral_on) && ( STATUS_CYCLE_RUNNING == true))
	{
...
{ 
	...
	{
		if (  (STATUS_WAIT_FOR_TEMP) && (state_TRAP_Cool<1) && ( Millis_CryoTimeout > HW_Cryotimeout_Millis) )		// on  and  too long
			{	
...
{		
	if ( ( state_TRAP_Cool > 0.9 ) && ( STATUS_CYCLE_RUNNING == true) )	// Calc for COOL State
	...
	// modify with user-defined slope  AND with heatUpSlope, determined in update_pid
	currentTemperature = currentTemperature / TempCorrectionSlope;// * heatUpSlope;
...
{	return (float)state_GC_Ready;				// Is OUT, so return the var
}	
...
{
	set_pin_TRAP_Cool_and_Consequences(OFF);
	if (IsVarianGC)		set_pin_GC_Ready(OFF);
	else				      set_pin_GC_Ready(ON);
}
